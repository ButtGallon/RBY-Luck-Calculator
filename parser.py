    ...
    for line in logfile:
        if line.find("|") > -1:
            ...
            line = line.replace(", M", "").replace(", F", "").replace("-*", "").replace(", shiny", "").replace(", L50",
                                                                                                               "").replace(
                ", L5", "").replace("-Striped", "").replace("-Trash", "").replace("-Sandy", "").replace("Indeedee-F",
                                                                                                        "Indeedee").replace(
                "-Super", "").replace("-Large", "").replace("-Small", "").replace("-Blue", "").replace("-Orange",
                                                                                                       "").replace(
                "Florges-White", "Florges").replace("-Pokeball", "").replace("-Elegant", "").replace("-Indigo",
                                                                                                     "").replace(
                "-Yellow", "").replace("-Bug", "").replace("-Dark", "").replace("-Dragon", "").replace(
                "Arceus-Electric", "Silvally").replace("Silvally-Electric", "Silvally").replace("-Fairy", "").replace(
                "-Fighting", "").replace("-Fire", "").replace("-Flying", "").replace("-Ghost", "").replace("-Grass",
                                                                                                           "").replace(
                "-Ground", "").replace("-Ice", "").replace("-Normal", "").replace("-Poison", "").replace(
                "Arceus-Psychic", "Arceus").replace("Silvally-Psychic", "Silvally").replace("-Rock", "").replace(
                "-Steel", "").replace("-Water", "").replace("-Douse", "").replace("-Burn", "").replace("-Chill",
                                                                                                       "").replace(
                "-Shock", "").replace("Type: ", "Type:").replace("Mr. ", "Mr.").replace("-Sensu", "").replace(
                "-Pom-Pom", "").replace("-Pa'u", "").replace("Farfetch'd", "Farfetchd").replace("-Totem", "").replace(
                "-Resolute", "").replace("-Meteor", "").replace("Meowstic-F", "Meowstic").replace("-East", "").replace(
                "fetch’d", "fetchd").replace("fetch'd", "fetchd").replace("-Ruby-Cream", "").replace("-Matcha-Cream",
                                                                                                     "").replace(
                "-Mint-Cream", "").replace("-Lemon-Cream", "").replace("-Salted-Cream", "").replace("-Ruby-Swirl",
                                                                                                    "").replace(
                "-Caramel-Swirl", "").replace("-Rainbow-Swirl", "")
            linestoremove = ["|", "|teampreview", "|clearpoke", "|upkeep"]
            badlines = ["", "|start", "|player|p1", "|player|p2", "|player|p1|", "|player|p2|", "|-notarget",
                        "|-clearallboost", "|-nothing", "|-ohko", "|rated"]
            linepurposestoremove = ["j", "c", "l", "html", "raw", "teamsize", "gen", "gametype", "tier", "rule",
                                    "-mega", "seed", "teampreview", "anim"]
                    ...
                    for segment in lineremainder.split("|"):
                        if segment.find("/") > -1:
                            try:
                                ...
                                segment = segment.replace(f"/{denomenator}", "/100").replace(f"{numerator}/",
                                                                                             f"{newnumerator}/")
                            except:
                                pass
...
def activate_function(line, parsedlogfile, results):
    if line[3].split("|")[1] == "confusion":
        ...
        killer['damagedone'] += fainted['remaininghealth'];
        killer['kills'] += 1
        fainted['remaininghealth'] = 0;
        fainted['deaths'] = 1
...
def damage_function(line, parsedlogfile, results):
    ...
    if line[3].find("[from]") > -1:
        ...
        if cause in ['Stealth Rock', 'Spikes', 'G-Max Steelsurge']:
        ...
        elif cause.find("item: Rocky Helmet") > -1 or cause.find("Leech Seed") > -1 or cause.find(
                "ability: Iron Barbs") > -1 or cause.find("ability: Rough Skin") > -1 or cause.find(
                "ability: Aftermath") > -1 or cause.find("ability: Liquid Ooze") > -1 or cause.find(
                "ability: Innards Out") > -1 or cause.find("ability: Bad Dreams") > -1 or cause.find(
                "ability: Gulp Missile") > -1 or cause.find("Spiky Shield") > -1 or cause.find("leechseed") > -1:
            damager = cause.split("|[of] ")[1].split(": ", 1)[1]
            team = cause.split("|[of] ")[1].split(": ", 1)[0]
            damager = roster_search(team, damager, results)
        elif cause.find("move: Whirlpool") > -1 or cause.find("move: Infestation") > -1 or cause.find(
                "move: Magma Storm") > -1 or cause.find("move: Wrap") > -1 or cause.find(
                "move: Fire Spin") > -1 or cause.find("move: Thunder Cage") > -1 or cause.find(
                "move: Bind") > -1 or cause.find("move: Sand Tomb") > -1 or cause.find(
                "ability: Disguise|[of]") > -1 or cause.find("mimikyubusted") > -1 or cause.find(
                "Mimikyu-Busted") > -1 or cause.find("G-Max Vine Lash") > -1:
            if team == "p1a":
        ...
        elif cause in ['Recoil', 'item: Life Orb', 'highjumpkick', 'recoil', 'High Jump Kick', 'Mind Blown',
                       'steelbeam', 'mindblown', 'Steel Beam'] or cause.find("Recoil|[of] ") > -1 or cause.find(
                "recoil|[of] ") > -1:
            pokemon['hphealed'] += -damagedone
            ...
            for line_ in matchdata:
                if line_[2] == "item" and line_[3].find(cause.split(": ", 1)[1]) > -1 and line_[3].find(
                        pokemon['nickname']) > -1:
                    switched = True
                if line_[2] == "move" and line_[3].split("|")[1] in ['Trick', 'Switcheroo'] and switched == True and \
                        line_[3].split("|")[0].split(": ", 1)[1] != pokemon['nickname']:
                    damager = line_[3].split("|")[0].split(": ", 1)[1]
                    team = line_[3].split(": ", 1)[0]
                    damager = roster_search(team, damager, results)
                    accounted = True
                    break
                elif line_[2] == "move" and line_[3].split("|")[1] in ['Trick', 'Switcheroo'] and switched == False and \
                        line_[3].split("|")[0].split(": ", 1)[1] == pokemon['nickname']:
                    pokemon['hphealed'] += -damagedone
                    accounted = True
                    break
...
def damager_search(parsedlogfile, line, team, pokemon, results, damagedone):
    ...
    for line in turndata:
        if team == "p1a":
            if line[2] == "end" and line[3].find(f"p1a: {pokemon['nickname']}") > -1 and line[3].find(
                    "|move: Future Sight") > -1:
                damager = roster_search("p2a", results['team1']['Future Sight'], results)
                damager['damagedone'] += damagedone
                move = "Future Sight"
                break
            elif line[2] == "end" and line[3].find(f"p1a: {pokemon['nickname']}") > -1 and line[3].find(
                    "|move: Doom Desire") > -1:
                damager = roster_search("p2a", results['team1']['Doom Desire'], results)
                damager['damagedone'] += damagedone
                move = "Doom Desire"
                break
            elif line[2] == "start" and line[3].find(f"p1a: {pokemon['nickname']}") > -1 and line[3].find(
                    "|Substitute") > -1:
                pokemon['hphealed'] += -damagedone
                break
            ...
            if line[2] == "end" and line[3].find(f"p2a: {pokemon['nickname']}") > -1 and line[3].find(
                    "|move: Future Sight") > -1:
                damager = roster_search("p1a", results['team2']['Future Sight'], results)
                damager['damagedone'] += damagedone
                move = "Future Sight"
                break
            elif line[2] == "end" and line[3].find(f"p2a: {pokemon['nickname']}") > -1 and line[3].find(
                    "|move: Doom Desire") > -1:
                damager = roster_search("p1a", results['team2']['Doom Desire'], results)
                damager['damagedone'] += damagedone
                move = "Doom Desire"
                break
            elif line[2] == "start" and line[3].find(f"p2a: {pokemon['nickname']}") > -1 and line[3].find(
                    "|Substitute") > -1:
                pokemon['hphealed'] += -damagedone
                break
...
def heal_function(line, parsedlogfile, results):
    ...
    if line[3].find("|[wisher] ") == -1 and line[3].find("[from] move: Lunar Dance") == -1 and line[3].find(
            "[from] move: Healing Wish") == -1:
...
def move_function(line, parsedlogfile, results):
    notothermove = True
    if line[3].find("[from]Magic Bounce") > -1 or line[3].find("[from]Copycat") > -1 or line[3].find(
            "[from]Metronome") > -1 or line[3].find("[from]Magic Coat") > -1 or line[3].find(
            "[from]Nature Power") > -1 or line[3].find("[from]Assist") > -1 or line[3].find("[from]Me First") > -1:
        notothermove = False
    # parse line
    move = line[3].split("|")[1]
    ...
    try:
        for line_ in turndata:
            if line_[2] == "immune" and line_[3].find(defendingteam) > -1 and line_[3].find(target['nickname']) > -1:
                notimmune = False
            if line_[2] == "ability" and line_[3].find(defendingteam) > -1 and line_[3].find(
                    target['nickname']) > -1 and line_[3].split("|")[2] in ['Storm Drain', 'Lightning Rod',
                                                                            'Sap Sipper', 'Flash Fire', 'Motor Drive']:
                notimmune = False
            if line_[2] == "damage" and line_[3].find(defendingteam) > -1 and line_[3].find(target['nickname']) > -1 and \
                ...
                targetalive = False
            if line_[2] == "activate" and line_[3].find(defendingteam) > -1 and line_[3].find(
                    target['nickname']) > -1 and line_[3].split("|")[1].split("move: ")[1] in ['Protect', 'Detect',
                                                                                               "King's Sheild",
                                                                                               'Spiky Sheild',
                                                                                               'Mat Block', 'Obstruct']:
        ...
        pass
    # support moves
    supportmoves = ['Reflect', 'Light Screen', 'Heal Bell', 'Aromatherapy', 'Wish', 'Stealth Rock', 'Spikes',
                    'Toxic Spikes', 'Sticky Web', 'Aurora Veil', 'Defog', 'Rapid Spin', 'Hail', 'Sandstorm',
                    'Sunny Day', 'Rain Dance', 'Encore', 'Taunt', 'Haze', 'Clear Smog', 'Roar', 'Whirlwind',
                    'Leech Seed', 'Toxic', 'Will-O-Wisp', 'Stun Spore', 'Poison Powder', 'Block', 'Mean Look',
                    'Dark Void', 'Destiny Bond', 'Disable', 'Electric Terrain', 'Embargo', 'Endure', 'Fairy Lock',
                    "Forest's Curse", 'Glare', 'Grass Whistle', 'Grassy Terrain', 'Gravity', 'Grudge', 'Heal Block',
                    'Healing Wish', 'Hypnosis', 'Lucky Chant', 'Lunar Dance', 'Magic Coat', 'Magic Room', 'Mean Look',
                    'Memento', 'Mist', 'Misty Terrain', 'Mud Sport', 'Parting Shot', 'Perish Song', 'Poison Gas',
                    'Psychic Terrain', 'Safeguard', 'Simple Beam', 'Sing', 'Skill Swap', 'Sleep Powder', 'Soak',
                    'Speed Swap', 'Spider Web', 'Spite', 'Spore', 'Sweet Kiss', 'Switcheroo', 'Tailwind',
                    'Thunder Wave', 'Torment', 'Toxic Thread', 'Trick', 'Trick Room', 'Water Sport', 'Wonder Room',
                    'Worry Seed', 'Yawn']
    # check for support
    ...
    movesthatcanmiss1 = [['Eerie Impulse', 100], ['Scratch', 100], ['Anchor Shot', 100], ['Avalanche', 100],
                         ['Sand Tomb', 85], ['Fire Punch', 100], ['Infestation', 100], ['Superpower', 100],
                         ['Zing Zap', 100], ['Giga Drain', 100], ['Chatter', 100], ['Ice Punch', 100],
                         ['Tail Slap', 85], ['Double Slap', 85], ['Thunder Shock', 100], ['Sky Uppercut', 90],
                         ['Super Fang', 90], ['Hyper Beam', 90], ['Dynamic Punch', 50], ['Poison Sting', 100],
                         ['Dig', 100], ['Focus Blast', 70], ['Liquidation', 100], ['Double Kick', 100],
                         ['Ice Ball', 90], ['Round', 100], ['False Swipe', 100], ['Crush Claw', 95], ['Leech Seed', 90],
                         ['Shadow Sneak', 100], ['Crush Grip', 100], ['Hidden Power Electric', 100], ['Seed Bomb', 100],
                         ['Hidden Power Fire', 100], ['Chip Away', 100], ['Metal Claw', 95], ['Feather Dance', 100],
                         ['Acid', 100], ['Baddy Bad', 100], ['Accelerock', 100], ['Aurora Beam', 100],
                         ['Heart Stamp', 100], ['Magma Storm', 75], ['Crunch', 100], ['Aeroblast', 95],
                         ['Explosion', 100], ['Thunder Wave', 90], ['Moongeist Beam', 100], ['Revenge', 100],
                         ['Wake-Up Slap', 100], ['Pursuit', 100], ['Head Smash', 80], ['Sing', 55], ['Earthquake', 100],
                         ['Gastro Acid', 100], ['Incinerate', 100], ['Water Pulse', 100], ['Dragon Pulse', 100],
                         ['Glitzy Glow', 100], ['Acid Spray', 100], ['Thunder Punch', 100], ['Extrasensory', 100],
                         ['Wring Out', 100], ["Forest's Curse", 100], ['Blizzard', 70], ['Blast Burn', 90],
                         ['Paleo Wave', 100], ['Mud-Slap', 100], ['Freeze-Dry', 100], ['Heavy Slam', 100],
                         ['Overheat', 90], ['Captivate', 100], ['Flame Charge', 100], ['Swagger', 85],
                         ['Dizzy Punch', 100], ['Flying Press', 95], ['Lunge', 100], ['Prismatic Laser', 100],
                         ['U-turn', 100], ['Future Sight', 100], ['Fire Lash', 100], ['Smelling Salts', 100],
                         ['Burn Up', 100], ['Bug Bite', 100], ['Poison Gas', 90], ['Hidden Power Ghost', 100],
                         ['Shell Trap', 100], ['Attract', 100], ['Sky Drop', 100], ['Crabhammer', 90],
                         ['Darkest Lariat', 100], ['Hidden Power', 100], ['Razor Leaf', 95], ['Poison Powder', 75],
                         ['Head Charge', 100], ['Dive', 100], ['Signal Beam', 100], ['Rock Tomb', 95],
                         ['Power Whip', 85], ['Buzzy Buzz', 100], ['Supersonic', 55], ['Psychic Fangs', 100],
                         ['Bind', 85], ['Counter', 100], ['Memento', 100], ['Throat Chop', 100],
                         ['Parabolic Charge', 100], ['Retaliate', 100], ['Final Gambit', 100], ['Steamroller', 100],
                         ['Bullet Seed', 100], ['Freezy Frost', 100], ['Reversal', 100], ['Aqua Jet', 100],
                         ['Grass Knot', 100], ['Fissure', 30], ['Flame Burst', 100], ['Thief', 100], ['Sweet Kiss', 75],
                         ['Astonish', 100], ['Dream Eater', 100], ['Energy Ball', 100], ['Fury Swipes', 80],
                         ['Return', 100], ['Fling', 100], ['Belch', 90], ['Power Trip', 100], ['Fury Cutter', 95],
                         ['Iron Tail', 75], ['Toxic', 90], ['Hidden Power Flying', 100], ['Egg Bomb', 75],
                         ['Payback', 100], ['Fusion Flare', 100], ['Flail', 100], ['Boomburst', 100],
                         ['Secret Sword', 100], ['Thunder Fang', 95], ['Charge Beam', 90], ['Lovely Kiss', 75],
                         ['Rapid Spin', 100], ['Skull Bash', 100], ['Clamp', 85], ['Rock Throw', 90],
                         ['Secret Power', 100], ['Fairy Wind', 100], ['Water Shuriken', 100], ['Struggle Bug', 100],
                         ['Ice Fang', 95], ['Self-Destruct', 100], ['Teeter Dance', 100], ['Stored Power', 100],
                         ['Tri Attack', 100], ['Psychic', 100], ['Electroweb', 95], ['Weather Ball', 100],
                         ['Whirlpool', 85], ['Smog', 70], ['Baby-Doll Eyes', 100], ['Assurance', 100], ['Icy Wind', 95],
                         ['Steel Wing', 90], ['Core Enforcer', 100], ['Synchronoise', 100], ['Zippy Zap', 100],
                         ['Petal Dance', 100], ['Iron Head', 100], ['Sonic Boom', 90], ['Trop Kick', 100],
                         ['Headbutt', 100], ['Drain Punch', 100], ['Hypnosis', 60], ['Dragon Rage', 100],
                         ['Bug Buzz', 100], ['Cotton Spore', 100], ['Fire Fang', 95], ['Sludge', 100],
                         ['Mist Ball', 100], ['Jump Kick', 95], ['Water Spout', 100], ['Ice Shard', 100],
                         ['Earth Power', 100], ['Stomping Tantrum', 100], ['Bolt Strike', 85], ['Bite', 100],
                         ['Sappy Seed', 100], ['Mega Kick', 75], ['Inferno', 50], ['Confuse Ray', 100],
                         ['Rock Climb', 85], ['Rock Blast', 90], ['Feint', 100], ['Sucker Punch', 100],
                         ['Dragon Ascent', 100], ['Freeze Shock', 90], ['Peck', 100], ['Psywave', 100],
                         ['Drill Run', 95], ['Leafage', 100], ['Dark Pulse', 100], ['Flash Cannon', 100],
                         ['Floaty Fall', 95], ['Poison Fang', 100], ['Dazzling Gleam', 100], ['Double-Edge', 100],
                         ['Hidden Power Rock', 100], ['Encore', 100], ['Sweet Scent', 100], ['Sludge Bomb', 100],
                         ['Lick', 100], ['Fire Spin', 85], ['Close Combat', 100], ['Bone Rush', 90], ['Guillotine', 30],
                         ['Slam', 75], ['Torment', 100], ['Aqua Tail', 90], ['Strength', 100], ['Soak', 100],
                         ['Fusion Bolt', 100], ['Rage', 100], ['Hidden Power Dark', 100], ['Metal Sound', 85],
                         ['Gyro Ball', 100], ['Psycho Boost', 90], ['Surf', 100], ['Taunt', 100], ['Gear Grind', 85],
                         ['Shadow Ball', 100], ['Eruption', 100], ['Barrage', 85], ['Charm', 100],
                         ['Shadow Force', 100], ['Bonemerang', 90], ['Cross Poison', 100], ['Low Sweep', 100],
                         ['Sludge Wave', 100], ['Leaf Tornado', 90], ['Hyper Fang', 90], ['Covet', 100],
                         ['Karate Chop', 100], ['Fell Stinger', 100], ['Thunder', 70], ['Constrict', 100],
                         ['Razor Wind', 100], ['Water Pledge', 100], ['Brine', 100], ['Hammer Arm', 90],
                         ['Last Resort', 100], ['Bullet Punch', 100], ['Spite', 100], ['Brick Break', 100],
                         ['Pay Day', 100], ['Switcheroo', 100], ['Oblivion Wing', 100], ['Origin Pulse', 85],
                         ['Kinesis', 80], ['Stomp', 100], ['Grass Whistle', 55], ['Poison Tail', 100],
                         ['Bone Club', 85], ['Fire Blast', 85], ['Brave Bird', 100], ['Gust', 100], ['Heat Crash', 100],
                         ['Sand Attack', 100], ['Mud Bomb', 85], ['Beat Up', 100], ['Leaf Blade', 100], ['Spore', 100],
                         ['Sparkly Swirl', 100], ['Trick', 100], ['Powder', 100], ['Twineedle', 100],
                         ['Solar Blade', 100], ['Splishy Splash', 100], ['Spirit Shackle', 100], ['Nightmare', 100],
                         ['Parting Shot', 100], ['Mystical Fire', 100], ['Confusion', 100], ['Pluck', 100],
                         ['Frost Breath', 90], ['Roar of Time', 90], ['Fury Attack', 85], ['High Jump Kick', 90],
                         ['Brutal Swing', 100], ['Petal Blizzard', 100], ['Wing Attack', 100],
                         ['Hidden Power Steel', 100], ['Wood Hammer', 100], ['Hidden Power Water', 100],
                         ['Fiery Dance', 100], ['Echoed Voice', 100], ['Smack Down', 100], ["Land's Wrath", 100],
                         ['Mega Punch', 85], ['Stun Spore', 75], ['Revelation Dance', 100], ['Night Daze', 95],
                         ['Scary Face', 100], ['Dragon Tail', 90], ['Horn Drill', 30], ['String Shot', 95],
                         ['Flare Blitz', 100], ['Heal Block', 100], ['Ice Hammer', 90], ['V-create', 95],
                         ['Screech', 85], ['Shadow Claw', 100], ['Sacred Fire', 95], ['Ice Beam', 100], ['Facade', 100],
                         ['Worry Seed', 100], ['Vice Grip', 100], ['Growl', 100], ['Hidden Power Ice', 100],
                         ['Extreme Speed', 100], ['Poison Jab', 100], ['Take Down', 85], ['Phantom Force', 100],
                         ['Double Iron Bash', 100], ['Mind Blown', 100], ['Multi-Attack', 100], ['Zap Cannon', 50],
                         ['Absorb', 100], ['Waterfall', 100], ['Judgment', 100], ['Hidden Power Bug', 100],
                         ['Psybeam', 100], ['Knock Off', 100], ['Octazooka', 85], ['Hyper Voice', 100],
                         ['Double Hit', 90], ['Spark', 100], ['Stone Edge', 80], ['Metal Burst', 100],
                         ['Rock Wrecker', 90], ['Bouncy Bubble', 100], ['Draining Kiss', 100], ['Mud Shot', 95],
                         ['Present', 90], ['Mirror Shot', 85], ['Megahorn', 85], ['Acrobatics', 100],
                         ['Fleur Cannon', 90], ['Dragon Rush', 75], ['Solar Beam', 100], ['Mach Punch', 100],
                         ['Luster Purge', 100], ['Snore', 100], ['Submission', 80], ['Vacuum Wave', 100],
                         ['Thunderbolt', 100], ['Circle Throw', 90], ['Flame Wheel', 100], ['Hidden Power Poison', 100],
                         ['Glare', 100], ['Flatter', 100], ['Fire Pledge', 100], ['Bounce', 85], ['Photon Geyser', 100],
                         ['Giga Impact', 90], ['Dragon Claw', 100], ['Endeavor', 100], ['Scald', 100], ['Uproar', 100],
                         ['Force Palm', 100], ['Hurricane', 70], ['Ice Burn', 90], ['Magnitude', 100],
                         ['Cross Chop', 80], ['Psyshock', 100], ['Hidden Power Grass', 100], ['Techno Blast', 100],
                         ['Gunk Shot', 80], ['Wild Charge', 100], ['Dragon Hammer', 100], ['Horn Attack', 100],
                         ['Noble Roar', 100], ['Blaze Kick', 90], ['Psystrike', 100], ['Spike Cannon', 100],
                         ['Horn Leech', 100], ['Searing Shot', 100], ['Dark Void', 50], ['Arm Thrust', 100],
                         ['Dual Chop', 90], ['Foul Play', 100], ['Twister', 100], ['Focus Punch', 100], ['Slash', 100],
                         ['Rollout', 90], ['Icicle Spear', 100], ['Wrap', 90], ['Leech Life', 100],
                         ['Sacred Sword', 100], ['Hex', 100], ['Spit Up', 100], ['Will-O-Wisp', 85], ['Bubble', 100],
                         ['Quick Attack', 100], ['Glaciate', 95], ['Night Shade', 100], ['Psycho Cut', 100],
                         ['Sizzly Slide', 100], ['Rock Smash', 100], ['Volt Tackle', 100], ['Grass Pledge', 100],
                         ['Sparkling Aria', 100], ['Embargo', 100], ['Vine Whip', 100], ['Ember', 100],
                         ['Spacial Rend', 95], ['Tail Whip', 100], ['Light of Ruin', 90], ['Powder Snow', 100],
                         ['Power-Up Punch', 100], ['Spectral Thief', 100], ['Smokescreen', 100], ['Moonblast', 100],
                         ['Leer', 100], ['Thousand Waves', 100], ['Rock Slide', 90], ['Drill Peck', 100],
                         ['Storm Throw', 100], ['Heat Wave', 90], ['Mirror Coat', 100], ['Tackle', 100],
                         ['Discharge', 100], ['Strength Sap', 100], ['Razor Shell', 95], ['Hidden Power Ground', 100],
                         ['Rolling Kick', 85], ['Blue Flare', 85], ['Sky Attack', 90], ['Power Gem', 100],
                         ['Mega Drain', 100], ['High Horsepower', 95], ['Seismic Toss', 100], ['Electro Ball', 100],
                         ['Flash', 100], ['Tickle', 100], ['Quash', 100], ['Venom Drench', 100], ['Pollen Puff', 100],
                         ['Triple Kick', 90], ['Night Slash', 100], ['Plasma Fists', 100], ['Punishment', 100],
                         ['Pin Missile', 95], ['First Impression', 100], ['Fly', 95], ['Natural Gift', 100],
                         ['Relic Song', 100], ['Fake Out', 100], ['Disable', 100], ['Ancient Power', 100],
                         ['Bubble Beam', 100], ['Play Rough', 90], ['Zen Headbutt', 90], ['Clanging Scales', 100],
                         ['Shadow Strike', 95], ['Hidden Power Dragon', 100], ['Body Slam', 100], ['Seed Flare', 85],
                         ['Leaf Storm', 90], ['Air Cutter', 95], ['Flamethrower', 100], ['Doom Desire', 100],
                         ['Sunsteel Strike', 100], ['Frenzy Plant', 90], ['Diamond Storm', 95], ['Draco Meteor', 90],
                         ['Muddy Water', 85], ['Sleep Powder', 75], ['Simple Beam', 100], ['Outrage', 100],
                         ['Sheer Cold', 30], ['Beak Blast', 100], ['Trick-or-Treat', 100]]
    movesthatcanmiss2 = [['Hydro Pump', 80], ['Steam Eruption', 95], ['X-Scissor', 100], ['Water Gun', 100],
                         ['Nuzzle', 100], ['Hidden Power Fighting', 100], ['Snarl', 95], ['Venoshock', 100],
                         ['Thousand Arrows', 100], ['Needle Arm', 100], ['Dragon Breath', 100], ['Cut', 95],
                         ['Comet Punch', 85], ['Fake Tears', 100], ['Low Kick', 100], ['Bulldoze', 100],
                         ['Psycho Shift', 100], ['Thrash', 100], ['Volt Switch', 100], ['Pound', 100],
                         ['Silver Wind', 100], ['Precipice Blades', 85], ['Toxic Thread', 100],
                         ['Hidden Power Psychic', 100], ['Hydro Cannon', 90], ['Air Slash', 95], ['Lava Plume', 100],
                         ['Meteor Mash', 90], ['Icicle Crash', 90], ['Frustration', 100], ['Attack Order', 100],
                         ['Entrainment', 100], ['Hold Back', 100], ["Nature's Madness", 90], ['Shadow Bone', 100],
                         ['Ominous Wind', 100], ['Steel Beam', 95], ['Apple Acid', 100], ['Body Press', 100],
                         ['Fishious Rend', 100], ['Bolt Beak', 100], ['Breaking Swipe', 100], ['Strange Steam', 95],
                         ['Vise Grip', 100], ['Jaw Lock', 100], ['Pyro Ball', 90], ['Tar Shot', 100],
                         ['Dynamax Cannon', 100], ['Snipe Shot', 100], ['Magic Powder', 100], ['Octolock', 100],
                         ['Drum Beating', 100], ['Snap Trap', 100], ['Behemoth Blade', 100], ['Branch Poke', 100],
                         ['Aura Wheel', 100], ['Overdrive', 100], ['Grav Apple', 100], ['Behemoth Bash', 100],
                         ['Spirit Break', 100], ['Meteor Assault', 100], ['Eternabeam', 90], ['Dragon Darts', 100]]
    ...
    if move in movesthatcanmiss.keys() and notimmune and targetnotprotected and targetmon and not semiinv:
        try:
            ...
            if attacker['pokemon'] in ['Venonat', 'Joltik', 'Yanma', 'Vivillon', 'Nincada', 'Dustox', 'Dottler',
                                       'Galvantula', 'Butterfree-Gmax', 'Scatterbug', 'Butterfree', 'Blipbug']:
                acc_modifier = acc_modifier * 1.3
            if attacker['pokemon'] in ['Victini']:
                acc_modifier = acc_modifier * 1.1
            # no guard
            if attacker['pokemon'] in ['Machamp', 'Pidgeot-Mega', 'Golett', 'Lycanroc-Midnight', 'Machoke', 'Golurk',
                                       'Honedge', 'Karrablast', 'Doublade', 'Machamp-Gmax', 'Machop']:
            ...
            if move in ['Blizzard'] and results['Hail']:
                acc_modifier = 100000000
            results = luckappend(line, results, attacker, f"Mon Used a Move That Can Miss ({move})",
                                 100 - min(movesthatcanmiss[move] * acc_modifier, 100))
            results = luckappend(line, results, target, f"Target of Move That Can Miss ({move})",
                                 -(100 - min(movesthatcanmiss[move] * acc_modifier, 100)))
        except:
            pass
    # check for miss
    ...
    if line[3].find("|[miss]") > -1 and notimmune and targetnotprotected and targetmon and not semiinv:
        ...
        movehit = False
    # check if can crit
    movesthatcancrit = ['Scratch', 'Anchor Shot', 'Avalanche', 'Sand Tomb', 'Fire Punch', 'Infestation', 'Superpower',
                        'Zing Zap', 'Giga Drain', 'Chatter', 'Ice Punch', 'Tail Slap', 'Double Slap', 'Thunder Shock',
                        'Splintered Stormshards', 'Sky Uppercut', 'Hyper Beam', 'Dynamic Punch', 'Poison Sting', 'Dig',
                        'Focus Blast', 'Liquidation', 'Double Kick', 'Ice Ball', 'Round', 'False Swipe', 'Crush Claw',
                        'Shadow Sneak', 'Tectonic Rage', 'Hidden Power Electric', 'Seed Bomb', 'Hidden Power Fire',
                        'Chip Away', 'Metal Claw', 'Acid', 'Vital Throw', 'Shock Wave', 'Baddy Bad', 'Accelerock',
                        'Aurora Beam', 'Heart Stamp', 'Magma Storm', 'Crunch', 'Aeroblast', 'Explosion',
                        'Gigavolt Havoc', 'Moongeist Beam', 'Revenge', 'Clear Smog', 'Wake-Up Slap', 'Pursuit',
                        'Head Smash', 'Earthquake', 'Incinerate', 'Water Pulse', 'Dragon Pulse', 'Glitzy Glow',
                        'Acid Spray', 'Thunder Punch', 'Extrasensory', 'Blizzard', 'Blast Burn', 'Paleo Wave',
                        'Mud-Slap', 'Inferno Overdrive', 'Freeze-Dry', 'Overheat', 'Savage Spin-Out', 'Flame Charge',
                        'Dizzy Punch', 'Flying Press', 'Lunge', 'Prismatic Laser', 'U-turn', 'Future Sight',
                        'Fire Lash', 'Smelling Salts', 'Burn Up', 'Catastropika', 'Bug Bite', 'Hidden Power Ghost',
                        'Shell Trap', 'Hyperspace Hole', 'Sky Drop', 'Crabhammer', 'Light That Burns the Sky',
                        'Darkest Lariat', 'Hidden Power', 'Razor Leaf', 'Head Charge', 'Dive', 'Signal Beam',
                        'Rock Tomb', 'Power Whip', 'Buzzy Buzz', "Let's Snuggle Forever", 'Psychic Fangs', 'Bind',
                        'Throat Chop', 'Parabolic Charge', 'Retaliate', 'Steamroller', 'Bullet Seed', 'Freezy Frost',
                        'Aqua Jet', 'Flame Burst', 'Thief', 'Astonish', 'Dream Eater', 'Energy Ball', 'Fury Swipes',
                        'Belch', 'Power Trip', 'Fury Cutter', 'Iron Tail', 'Hidden Power Flying', 'Egg Bomb', 'Payback',
                        'Fusion Flare', 'Boomburst', 'Secret Sword', 'Thunder Fang', 'Charge Beam', 'Rapid Spin',
                        'Skull Bash', 'Clamp', 'Rock Throw', 'Secret Power', 'Fairy Wind', 'Water Shuriken',
                        'Struggle Bug', 'Ice Fang', 'Self-Destruct', 'Sinister Arrow Raid', 'Stored Power',
                        'Tri Attack', 'Psychic', 'Electroweb', 'Weather Ball', 'Shattered Psyche', 'Whirlpool', 'Smog',
                        'Assurance', 'Icy Wind', 'Steel Wing', 'Core Enforcer', 'Synchronoise', 'Zippy Zap',
                        'Petal Dance', 'Iron Head', 'Trop Kick', 'Headbutt', 'Drain Punch', 'Bug Buzz', 'Fire Fang',
                        'Sludge', 'Mist Ball', 'Jump Kick', 'Water Spout', 'Ice Shard', 'Earth Power',
                        'Stomping Tantrum', 'Bolt Strike', 'Bite', 'Sappy Seed', 'Mega Kick', 'Inferno', 'Rock Climb',
                        'Rock Blast', 'Feint', 'Sucker Punch', 'Dragon Ascent', 'Freeze Shock', 'Peck', 'Drill Run',
                        'Leafage', 'Dark Pulse', 'Flash Cannon', 'Floaty Fall', 'Poison Fang', 'Dazzling Gleam',
                        'Double-Edge', 'Hidden Power Rock', 'Sludge Bomb', 'Lick', 'Fire Spin', 'Close Combat',
                        'Bone Rush', 'Slam', 'Aqua Tail', 'Strength', 'Fusion Bolt', 'Rage', 'Hidden Power Dark',
                        'Psycho Boost', 'Surf', 'Gear Grind', 'Shadow Ball', 'Eruption', 'Barrage', 'Shadow Force',
                        'Bonemerang', 'Cross Poison', 'Low Sweep', 'Sludge Wave', 'Leaf Tornado', 'Hyper Fang', 'Covet',
                        'Karate Chop', 'Fell Stinger', 'Thunder', 'Constrict', 'Razor Wind', 'Water Pledge', 'Brine',
                        'Hammer Arm', 'Last Resort', 'Pulverizing Pancake', 'Bullet Punch', 'Brick Break', 'Pay Day',
                        'Oblivion Wing', 'Origin Pulse', 'Stomp', 'Poison Tail', 'Bone Club', 'Fire Blast',
                        'Brave Bird', 'Gust', 'Mud Bomb', 'Leaf Blade', 'Sparkly Swirl', 'Twineedle', 'Solar Blade',
                        'Splishy Splash', 'Spirit Shackle', 'Continental Crush', 'Mystical Fire', 'Confusion', 'Pluck',
                        'Frost Breath', 'Roar of Time', 'Fury Attack', 'Swift', 'High Jump Kick', 'Brutal Swing',
                        'Petal Blizzard', 'Wing Attack', 'Hidden Power Steel', 'Wood Hammer', 'Hidden Power Water',
                        'Hyperspace Fury', 'Fiery Dance', 'Echoed Voice', 'Smack Down', "Land's Wrath", 'Mega Punch',
                        'Revelation Dance', 'Night Daze', 'Dragon Tail', 'Flare Blitz', 'Ice Hammer', 'V-create',
                        'Acid Downpour', 'Shadow Claw', 'Sacred Fire', 'Ice Beam', 'Facade', 'Vice Grip',
                        'Hidden Power Ice', 'Extreme Speed', 'Poison Jab', 'Take Down', 'Phantom Force',
                        'Double Iron Bash', 'Mind Blown', 'Multi-Attack', 'Zap Cannon', 'Magnet Bomb', 'Absorb',
                        'Waterfall', 'Judgment', 'Hidden Power Bug', 'Psybeam', 'Knock Off', 'Octazooka', 'Hyper Voice',
                        'Double Hit', 'Spark', 'Stone Edge', 'Rock Wrecker', 'Bouncy Bubble', 'Draining Kiss',
                        'Mud Shot', 'Mirror Shot', 'Megahorn', 'Acrobatics', 'Fleur Cannon', 'Dragon Rush',
                        'Solar Beam', 'Mach Punch', 'Luster Purge', 'Breakneck Blitz', 'Snore', 'Submission',
                        'Vacuum Wave', 'Thunderbolt', 'Circle Throw', 'All-Out Pummeling', 'Flame Wheel',
                        'Hidden Power Poison', 'Subzero Slammer', 'Fire Pledge', 'Bounce', 'Photon Geyser',
                        'Giga Impact', 'Dragon Claw', 'Soul-Stealing 7-Star Strike', 'Scald', 'Uproar', 'Force Palm',
                        'Hurricane', 'Ice Burn', 'Cross Chop', 'Psyshock', 'Hidden Power Grass', 'Techno Blast',
                        'Gunk Shot', 'Wild Charge', 'Dragon Hammer', 'Horn Attack', 'Blaze Kick', 'Psystrike',
                        'Spike Cannon', 'Horn Leech', 'Searing Shot', 'Supersonic Skystrike', 'Arm Thrust', 'Dual Chop',
                        'Foul Play', 'Twister', 'Focus Punch', 'Slash', 'Rollout', 'Icicle Spear', 'Wrap', 'Leech Life',
                        'Sacred Sword', 'Hex', 'Bubble', 'Quick Attack', 'Glaciate', 'Psycho Cut', 'Sizzly Slide',
                        'Rock Smash', 'Volt Tackle', 'Grass Pledge', 'Sparkling Aria', 'Vine Whip', 'Ember',
                        'Spacial Rend', 'Light of Ruin', 'Stoked Sparksurfer', 'Powder Snow', 'Power-Up Punch',
                        'Spectral Thief', 'Moonblast', 'Hydro Vortex', 'Thousand Waves', 'Rock Slide', 'Aura Sphere',
                        'Drill Peck', 'Storm Throw', 'Heat Wave', 'Tackle', 'Discharge', 'Razor Shell',
                        'Hidden Power Ground', 'Rolling Kick', 'Blue Flare', 'Sky Attack', 'Power Gem', 'Mega Drain',
                        'Genesis Supernova', 'High Horsepower', 'Black Hole Eclipse', 'Pollen Puff', 'Triple Kick',
                        'Night Slash', 'Plasma Fists', 'Pin Missile', 'First Impression', 'Fly', 'Relic Song',
                        'Fake Out', 'Ancient Power', 'Bubble Beam', 'Play Rough', 'Zen Headbutt', 'Clanging Scales',
                        'Shadow Strike', 'Hidden Power Dragon', 'Corkscrew Crash', 'Struggle', 'Body Slam',
                        'Seed Flare', 'Leaf Storm', 'Air Cutter', 'Flamethrower', 'Doom Desire', 'Sunsteel Strike',
                        'Twinkle Tackle', 'Frenzy Plant', 'Diamond Storm', 'Never-Ending Nightmare', 'Draco Meteor',
                        'Muddy Water', 'Outrage', 'Malicious Moonsault', 'Beak Blast', 'Hydro Pump', 'Steam Eruption',
                        'Smart Strike', 'X-Scissor', 'Disarming Voice', 'Water Gun', 'Clangorous Soulblaze',
                        "Magikarp's Revenge", 'Nuzzle', 'Hidden Power Fighting', 'Snarl', 'Venoshock',
                        'Thousand Arrows', 'Needle Arm', 'Dragon Breath', 'Searing Sunraze Smash', 'Cut', 'Comet Punch',
                        'Aerial Ace', 'Bulldoze', 'Menacing Moonraze Maelstrom', 'Thrash', 'Volt Switch', 'Pound',
                        'Silver Wind', 'Precipice Blades', 'Bloom Doom', 'Hidden Power Psychic', 'Hydro Cannon',
                        'Air Slash', 'Magical Leaf', '10,000,000 Volt Thunderbolt', 'Lava Plume', 'Meteor Mash',
                        'Shadow Punch', 'Feint Attack', 'Icicle Crash', 'Devastating Drake', 'Oceanic Operetta',
                        'Attack Order', 'Hold Back', 'Shadow Bone', 'Ominous Wind', 'Steel Beam', 'Apple Acid',
                        'Body Press', 'Fishious Rend', 'Bolt Beak', 'Breaking Swipe', 'Strange Steam', 'Vise Grip',
                        'Jaw Lock', 'Pyro Ball', 'Dynamax Cannon', 'Snipe Shot', 'Drum Beating', 'Snap Trap',
                        'Behemoth Blade', 'Branch Poke', 'Aura Wheel', 'Overdrive', 'Grav Apple', 'Behemoth Bash',
                        'Spirit Break', 'False Surrender', 'Meteor Assault', 'Eternabeam', 'Dragon Darts']
    increasedcrit = ['Aeroblast', 'Air Cutter', 'Attack Order', 'Blaze Kick', 'Crabhammer', 'Cross Chop',
                     'Cross Poison', 'Drill Run', 'Karate Chop', 'Leaf Blade', 'Night Slash', 'Poison Tail',
                     'Psycho Cut', 'Razor Leaf', 'Razor Wind', 'Shadow Claw', 'Sky Attack', 'Slash', 'Snipe Shot',
    ...
    if attacker['pokemon'] in ['Mareanie', 'Toxapex'] and (target['psn'] != None or target['tox'] != None):
        notmerciless = False
    if move in movesthatalwayscrit and notimmune and targetnotprotected and targetmon and movehit and target[
        'pokemon'] not in ['Type:Null', 'Slowbro-Mega', 'Turtonator']:
    ...
    elif move in increasedcrit and notimmune and targetnotprotected and targetmon and movehit and notmerciless and \
            target['pokemon'] not in ['Type:Null', 'Slowbro-Mega', 'Turtonator']:
        try:
            if attacker['Focus Energy']:
                results = luckappend(line, results, attacker,
                                     f"Mon With Focus Energy Used Move With Increased Crit Chance ({move})", -100)
                results = luckappend(line, results, target,
                                     f"Target of Mon With Focus Energy Using Move With Increased Crit Chance ({move})",
                                     100)
            elif attacker['pokemon'] in ['Unfezant', 'Pidove', 'Honchkrow', 'Tranquill', 'Togepi', 'Murkrow', 'Absol']:
                results = luckappend(line, results, attacker,
                                     f"Super Luck Mon Used Move With Increased Crit Chance ({move})", -50)
                results = luckappend(line, results, target,
                                     f"Target of Super Luck Mon Using Move With Increased Crit Chance ({move})", 50)
            else:
                results = luckappend(line, results, attacker, f"Mon Used Move With Increased Crit Chance ({move})",
                                     -12.5)
                results = luckappend(line, results, target, f"Target of Move With Increased Crit Chance ({move})", 12.5)
        except Exception as e:
            print(e)
    elif move in movesthatcancrit and notimmune and targetnotprotected and targetmon and movehit and notmerciless and \
            target['pokemon'] not in ['Type:Null', 'Slowbro-Mega', 'Turtonator']:
        try:
            if attacker['Focus Energy']:
                results = luckappend(line, results, attacker,
                                     f"Mon With Focus Energy Used Move With Crit Chance ({move})", -50)
                results = luckappend(line, results, target,
                                     f"Target of Mon With Focus Energy Using Move With Crit Chance ({move})", 50)
            elif attacker['pokemon'] in ['Unfezant', 'Pidove', 'Honchkrow', 'Tranquill', 'Togepi', 'Murkrow', 'Absol']:
                results = luckappend(line, results, attacker, f"Super Luck Mon Used Move With Crit Chance ({move})",
                                     -12.5)
                results = luckappend(line, results, target,
                                     f"Target of Super Luck Mon Using Move With Crit Chance ({move})", 12.5)
            else:
    ...
    statusdict = dict([['Thunder Punch', 'par'], ['Fire Punch', 'brn'], ['Fire Fang', 'brn'], ['Pyro Ball', 'brn'],
                       ['Ice Punch', 'frz'], ['Scorching Sands', 'brn'], ['Ice Fang', 'frz'], ['Thunder Fang', 'par'],
                       ['Thunder Shock', 'par'], ['Poison Sting', 'psn'], ['Blizzard', 'frz'], ['Freeze-Dry', 'frz'],
                       ['Relic Song', 'slp'], ['Sludge', 'psn'], ['Secret Power', 'par'], ['Smog', 'psn'],
                       ['Body Slam', 'par'], ['Bolt Strike', 'par'], ['Flamethrower', 'brn'], ['Freeze Shock', 'par'],
                       ['Poison Fang', 'tox'], ['Sludge Bomb', 'psn'], ['Lick', 'par'], ['Cross Poison', 'psn'],
                       ['Sludge Wave', 'psn'], ['Thunder', 'par'], ['Poison Tail', 'psn'], ['Fire Blast', 'brn'],
                       ['Twineedle', 'psn'], ['Splishy Splash', 'par'], ['Steam Eruption', 'brn'],
                       ['Flare Blitz', 'brn'], ['Sacred Fire', 'brn'], ['Ice Beam', 'frz'], ['Poison Jab', 'psn'],
                       ['Spark', 'par'], ['Dragon Breath', 'par'], ['Thunderbolt', 'par'], ['Flame Wheel', 'brn'],
                       ['Bounce', 'par'], ['Scald', 'brn'], ['Force Palm', 'par'], ['Ice Burn', 'brn'],
                       ['Gunk Shot', 'psn'], ['Blaze Kick', 'brn'], ['Lava Plume', 'brn'], ['Searing Shot', 'brn'],
                       ['Volt Tackle', 'par'], ['Ember', 'brn'], ['Powder Snow', 'frz'], ['Heat Wave', 'brn'],
                       ['Discharge', 'par'], ['Blue Flare', 'brn']])
    immunebytype = {
        'par': ['Yamper', 'Pichu', 'Electivire', 'Galvantula', 'Magnemite', 'Boltund', 'Golem-Alola', 'Stunfisk',
                'Flaaffy', 'Plusle', 'Thundurus', 'Eelektrik', 'Ampharos', 'Manectric-Mega', 'Luxray', 'Magnezone',
                'Manectric', 'Ampharos-Mega', 'Voltorb', 'Rotom-Frost', 'Electabuzz', 'Minun', 'Shinx', 'Zebstrika',
                'Rotom', 'Tynamo', 'Electrike', 'Toxel', 'Toxtricity', 'Joltik', 'Zeraora', 'Magneton', 'Blitzle',
                'Electrode', 'Elekid', 'Graveler-Alola', 'Charjabug', 'Zapdos', 'Rotom-Mow', 'Luxio', 'Zekrom',
                'Xurkitree', 'Geodude-Alola', 'Raikou', 'Pincurchin', 'Morpeko', 'Dracozolt', 'Arctozolt', 'Chinchou',
                'Mareep', 'Togedemaru', 'Pikachu-Gmax', 'Toxtricity-Gmax', 'Rotom-Wash', 'Thundurus-Therian',
                'Pachirisu', 'Heliolisk', 'Rotom-Heat', 'Jolteon', 'Rotom-Fan', 'Vikavolt', 'Raichu-Alola', 'Lanturn',
                'Pikachu', 'Emolga', 'Tapu Koko', 'Dedenne', 'Helioptile', 'Raichu', 'Eelektross'],
        'brn': ['Scorbunny', 'Raboot', 'Cinderace', 'Quilava', 'Tepig', 'Charmander', 'Salandit', 'Darumaka',
                'Combusken', 'Delphox', 'Talonflame', 'Moltres', 'Turtonator', 'Carkol', 'Coalossal', 'Pansear',
                'Simisear', 'Infernape', 'Heatran', 'Monferno', 'Reshiram', 'Litten', 'Braixen', 'Pyroar', 'Chandelure',
                'Litwick', 'Rapidash', 'Blacephalon', 'Arcanine', 'Volcarona', 'Fletchinder', 'Lampent',
                'Charizard-Mega-X', 'Oricorio', 'Ninetales', 'Ho-Oh', 'Slugma', 'Growlithe', 'Litleo', 'Torkoal',
                'Flareon', 'Darmanitan', 'Ponyta', 'Houndour', 'Sizzlipede', 'Centiskorch', 'Charmeleon', 'Camerupt',
                'Victini', 'Heatmor', 'Magmortar', 'Numel', 'Houndoom-Mega', 'Emboar', 'Houndoom', 'Marowak-Alola',
                'Magcargo', 'Blaziken', 'Incineroar', 'Chimchar', 'Volcanion', 'Coalossal-Gmax', 'Centiskorch-Gmax',
                'Charizard-Gmax', 'Magby', 'Charizard', 'Fennekin', 'Cyndaquil', 'Magmar', 'Groudon-Primal', 'Pignite',
                'Blaziken-Mega', 'Rotom-Heat', 'Entei', 'Charizard-Mega-Y', 'Vulpix', 'Torracat', 'Larvesta', 'Torchic',
                'Salazzle', 'Typhlosion', 'Camerupt-Mega'],
        'psn': ['Weezing-Galar', 'Salandit', 'Venusaur-Mega', 'Weezing', 'Qwilfish', 'Roselia', 'Gastly',
                'Beedrill-Mega', 'Grimer', 'Seviper', 'Scolipede', 'Zubat', 'Toxicroak', 'Tentacruel', 'Foongus',
                'Weepinbell', 'Stunky', 'Venusaur', 'Muk-Alola', 'Croagunk', 'Gloom', 'Koffing', 'Bellsprout',
                'Mareanie', 'Swalot', 'Nidoran-F', 'Nidoran-M', 'Skrelp', 'Crobat', 'Venipede', 'Amoonguss',
                'Gengar-Mega', 'Nidoking', 'Toxel', 'Toxtricity', 'Vileplume', 'Drapion', 'Haunter', 'Weedle',
                'Skorupi', 'Muk', 'Skuntank', 'Grimer-Alola', 'Garbodor', 'Toxapex', 'Ivysaur', 'Nihilego', 'Gulpin',
                'Trubbish', 'Golbat', 'Whirlipede', 'Eternatus', 'Beedrill', 'Spinarak', 'Budew', 'Nidoqueen',
                'Naganadel', 'Gengar', 'Toxtricity-Gmax', 'Garbodor-Gmax', 'Gengar-Gmax', 'Oddish', 'Bulbasaur',
                'Dustox', 'Arbok', 'Venonat', 'Dragalge', 'Roserade', 'Venomoth', 'Nidorino', 'Nidorina', 'Poipole',
                'Tentacool', 'Victreebel', 'Salazzle', 'Ariados', 'Ekans', 'Kakuna'],
        'tox': ['Weezing-Galar', 'Salandit', 'Venusaur-Mega', 'Weezing', 'Qwilfish', 'Roselia', 'Gastly',
                ...
                'Dustox', 'Arbok', 'Venonat', 'Dragalge', 'Roserade', 'Venomoth', 'Nidorino', 'Nidorina', 'Poipole',
                'Tentacool', 'Victreebel', 'Salazzle', 'Ariados', 'Ekans', 'Kakuna'],
        'frz': ['Mr.Mime-Galar', 'Darumaka-Galar', 'Darmanitan-Galar', 'Vanillish', 'Jynx', 'Swinub', 'Crabominable',
                'Kyurem-White', 'Kyurem', 'Sneasel', 'Glalie', 'Glaceon', 'Sandslash-Alola', 'Rotom-Frost', 'Abomasnow',
                'Abomasnow-Mega', 'Spheal', 'Ninetales-Alola', 'Sealeo', 'Snorunt', 'Vanilluxe', 'Cubchoo', 'Dewgong',
                'Sandshrew-Alola', 'Cloyster', 'Mr.Rime', 'Snom', 'Frosmoth', 'Eiscue', 'Arctozolt', 'Arctovish',
                'Glalie-Mega', 'Vanillite', 'Piloswine', 'Lapras-Gmax', 'Snover', 'Aurorus', 'Walrein', 'Smoochum',
                'Amaura', 'Avalugg', 'Weavile', 'Cryogonal', 'Delibird', 'Articuno', 'Vulpix-Alola', 'Mamoswine',
                'Beartic', 'Bergmite', 'Kyurem-Black', 'Froslass', 'Lapras', 'Regice'],
        'slp': [],
    }
    ...
    if move in statusdict.keys():
        ...
        if target['pokemon'] in immunebytype[status] and (
                attacker['pokemon'] not in ['Salazzle', 'Salandit'] or status not in ['psn', 'tox']):
            notimmunebytype = False
    if movedfirst:
        moveswithsecondaryeffect = dict(
            [['Shell Side Arm', 20], ['Scorching Sands', 30], ['Fire Punch', 10], ['Fire Fang', 19], ['Ice Fang', 19],
             ['Thunder Fang', 19], ['Zing Zap', 30], ['Ice Punch', 10], ['Thunder Shock', 10], ['Poison Sting', 30],
             ['Focus Blast', 10], ['Liquidation', 20], ['Crush Claw', 50], ['Metal Claw', 10], ['Acid', 10],
             ['Aurora Beam', 10], ['Heart Stamp', 30], ['Crunch', 20], ['Water Pulse', 20], ['Thunder Punch', 10],
             ['Extrasensory', 10], ['Blizzard', 10], ['Paleo Wave', 20], ['Freeze-Dry', 10], ['Dizzy Punch', 20],
             ['Signal Beam', 10], ['Steamroller', 30], ['Astonish', 30], ['Energy Ball', 10], ['Iron Tail', 30],
             ['Charge Beam', 70], ['Secret Power', 30], ['Tri Attack', 20], ['Psychic', 10], ['Smog', 40],
             ['Steel Wing', 10], ['Iron Head', 30], ['Headbutt', 30], ['Bug Buzz', 10], ['Sludge', 30],
             ['Mist Ball', 50], ['Earth Power', 10], ['Bolt Strike', 20], ['Bite', 30], ['Rock Climb', 20],
             ['Freeze Shock', 30], ['Dark Pulse', 20], ['Flash Cannon', 10], ['Floaty Fall', 30], ['Poison Fang', 50],
             ['Sludge Bomb', 30], ['Lick', 30], ['Shadow Ball', 20], ['Cross Poison', 10], ['Sludge Wave', 10],
             ['Leaf Tornado', 50], ['Hyper Fang', 10], ['Thunder', 30], ['Constrict', 10], ['Stomp', 30],
             ['Poison Tail', 10], ['Bone Club', 10], ['Fire Blast', 10], ['Mud Bomb', 30], ['Twineedle', 20],
             ['Splishy Splash', 30], ['Confusion', 10], ['Fiery Dance', 50], ['Night Daze', 40], ['Flare Blitz', 10],
             ['Sacred Fire', 50], ['Ice Beam', 10], ['Poison Jab', 30], ['Double Iron Bash', 51], ['Waterfall', 20],
             ['Psybeam', 10], ['Octazooka', 50], ['Spark', 30], ['Mirror Shot', 30], ['Dragon Rush', 20],
             ['Luster Purge', 50], ['Snore', 30], ['Thunderbolt', 10], ['Flame Wheel', 10], ['Bounce', 30],
             ['Scald', 30], ['Force Palm', 30], ['Hurricane', 30], ['Ice Burn', 30], ['Gunk Shot', 30],
             ['Blaze Kick', 10], ['Searing Shot', 30], ['Twister', 20], ['Bubble', 10], ['Rock Smash', 50],
             ['Volt Tackle', 10], ['Ember', 10], ['Powder Snow', 10], ['Moonblast', 30], ['Rock Slide', 30],
             ['Heat Wave', 10], ['Discharge', 30], ['Razor Shell', 50], ['Rolling Kick', 30], ['Blue Flare', 20],
             ['Sky Attack', 30], ['Relic Song', 10], ['Ancient Power', 10], ['Bubble Beam', 10], ['Play Rough', 10],
             ['Zen Headbutt', 20], ['Shadow Strike', 50], ['Body Slam', 30], ['Seed Flare', 40], ['Flamethrower', 10],
             ['Diamond Storm', 50], ['Muddy Water', 30], ['Steam Eruption', 30], ['Needle Arm', 30],
             ['Dragon Breath', 30], ['Silver Wind', 10], ['Air Slash', 30], ['Lava Plume', 30], ['Meteor Mash', 20],
             ['Icicle Crash', 30], ['Shadow Bone', 20], ['Ominous Wind', 10], ['Strange Steam', 20], ['Pyro Ball', 10]])
    else:
        moveswithsecondaryeffect = dict(
            [['Shell Side Arm', 20], ['Scorching Sands', 30], ['Fire Punch', 10], ['Fire Fang', 10], ['Ice Fang', 10],
             ['Thunder Fang', 10], ['Zing Zap', 30], ['Ice Punch', 10], ['Thunder Shock', 10], ['Poison Sting', 30],
             ['Focus Blast', 10], ['Liquidation', 20], ['Crush Claw', 50], ['Metal Claw', 10], ['Acid', 10],
             ['Aurora Beam', 10], ['Heart Stamp', 30], ['Crunch', 20], ['Water Pulse', 20], ['Thunder Punch', 10],
             ['Extrasensory', 10], ['Blizzard', 10], ['Paleo Wave', 20], ['Freeze-Dry', 10], ['Dizzy Punch', 20],
             ['Signal Beam', 10], ['Steamroller', 30], ['Astonish', 30], ['Energy Ball', 10], ['Iron Tail', 30],
             ...
             ['Dragon Breath', 30], ['Silver Wind', 10], ['Air Slash', 30], ['Lava Plume', 30], ['Meteor Mash', 20],
             ['Icicle Crash', 30], ['Shadow Bone', 20], ['Ominous Wind', 10], ['Strange Steam', 20], ['Pyro Ball', 10]])
    flinchmoves = ['Zing Zap', 'Heart Stamp', 'Extrasensory', 'Steamroller', 'Astonish', 'Iron Head', 'Headbutt',
                   'Bite', 'Dark Pulse', 'Floaty Fall', 'Hyper Fang', 'Stomp', 'Bone Club', 'Waterfall',
                   'Double Iron Bash', 'Dragon Rush', 'Snore', 'Twister', 'Rock Slide', 'Rolling Kick', 'Sky Attack',
                   'Fake Out', 'Zen Headbutt', 'Needle Arm', 'Air Slash', 'Icicle Crash']
    selfboostmoves = ['Metal Claw', 'Flame Charge', 'Charge Beam', 'Steel Wing', 'Fiery Dance', 'Power-Up Punch',
                      'Ancient Power', 'Diamond Storm', 'Silver Wind', 'Meteor Mash', 'Ominous Wind']
    if move in moveswithsecondaryeffect.keys() and notimmune and notstatused and targetnotprotected and targetmon and notimmunebytype and movehit and (
            targetalive or move in selfboostmoves) and (movedfirst or move not in flinchmoves):
        secondaryeffectmodifier = 1
        if attacker['pokemon'] in ['Chansey', 'Blissey', 'Dunsparce', 'Togepi', 'Togetic', 'Togekiss', 'Shaymin-Sky',
                                   'Jirachi', 'Happiny', 'Deerling', 'Sawsbuck', 'Meloetta']:
            secondaryeffectmodifier = 2
        try:
            results = luckappend(line, results, attacker, f"Mon Used Move With Secondary Effect Chance ({move})",
                                 -moveswithsecondaryeffect[move] * secondaryeffectmodifier)
            results = luckappend(line, results, target, f"Target of Move With Secondary Effect Chance ({move})",
                                 moveswithsecondaryeffect[move] * secondaryeffectmodifier)
    ...
    if move.replace("Z-", "") in ["Belly Drum", "Clangorous Soul"]:
        ...
        active = False
        for line_ in turndata:
            if (line_[2] == "setboost" and line_[3].find("[from] move: Belly Drum") > -1) or (
                    line_[2] == "boost" and line_[3].find("|def|1") > -1):
                active = True
        for line_ in turndata:
    ...
    if move.replace("Z-", "") in ['Final Gambit', 'Healing Wish', "Lunar Dance", "Memento", "Explosion",
                                  "Self-Destruct", 'Misty Explosion']:
        move = move.replace("Z-", "")
        # search for miss
        turndata = list(filter(lambda x: x[1] == line[1] and x[0] > line[0], parsedlogfile))[::-1]
        miss = False
...
def poke_function(line, parsedlogfile, results):
    ...
    if line[3].split("|", 1)[0] == "p1":
        results['team1']['roster'].append({
            ...
            'Curse': None, 'atk': 0, 'def': 0, 'spa': 0, 'spd': 0, 'spe': 0, 'accuracy': 0, 'evasion': 0,
            'Focus Energy': False, 'semiinv': False, 'moves': {}
        })
    elif line[3].split("|", 1)[0] == "p2":
    ...
    for line_ in replacelines:
        ...
        for line in relevantlines:
            if line[2] in ['switch', 'drag'] and line[3].find(montoreplace) > -1:
                ...
                line[3] = line[3].replace(f"{replaceteam}: {montoreplace}",
                                          f"{replaceteam}: {replacenickname}").replace(monname, pokemon)
                break
            else:
...
def status_function(line, parsedlogfile, results):
    ...
    if line[3].find("[from] ability: ") > -1:
        ...
        switched = False
        for line_ in matchdata:
            if line_[2] == "item" and line_[3].find(
                    mon['nickname']) > -1:  # and line_[3].find(cause.split(": ",1)[1])>-1
                switched = True
            if line_[2] == "move" and line_[3].split("|")[1] in ['Trick', 'Switcheroo'] and switched == True and \
                    line_[3].split("|")[0].split(": ", 1)[1] != mon['nickname']:
                damager = line_[3].split("|")[0].split(": ", 1)[1]
                mon[status] = damager
                break
            elif line_[2] == "move" and line_[3].split("|")[1] in ['Trick', 'Switcheroo'] and switched == True and \
                    line_[3].split("|")[0].split(": ", 1)[1] == mon['nickname']:
        ...
        movesthatcausestatus = dict([
            ...
            ['psn', ['Toxic Thread', 'Poison Powder', 'Poison Gas', 'Baneful Bunker', 'Fling', 'Psycho Shift',
                     'G-Max Stun Shock', 'G-Max Malodor']],
            ['brn', ['Beak Blast', 'Will-O-Wisp', 'Fling', 'Psycho Shift', 'Burning Jealousy']],
            ['par', ['Thunder Wave', 'Nuzzle', 'Glare', 'Stoked Sparksurfer', 'Stun Spore', 'Zap Cannon', 'Fling',
                     'Psycho Shift', 'G-Max Stun Shock']],
    ...
    if line[3].split("|")[1] == "confusion":
        ...
        if line[3].find("[fatigue]") > -1:
            ...
            if mon_['confusion'] == None:
                # check for berry
                turndata = list(
                    filter(lambda x: x[0] < line[0] and x[1] == line[1] and x[2] == "enditem", parsedlogfile))[::-1]
                for line_ in turndata:
